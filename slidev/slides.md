...
# s15-s18: 从单 Agent 到多 Agent 团队

> 从一次性子任务，到有身份、有协议、能自治、互不干扰的持久团队

<div class="grid grid-cols-2 gap-4 mt-8">

<div v-click class="p-4 rounded-xl bg-blue-500/10 border-2 border-blue-400/60">
  <div class="text-blue-300 font-bold text-base mb-2">s15 智能体团队</div>
  <div class="text-sm text-gray-300 space-y-1">
    <div>Subagent 用完就消失，复杂项目需要<span class="text-blue-300">持久队友</span></div>
    <div>每个队友有名字、角色、独立 loop 和 JSONL 邮箱</div>
    <div>Lead 通过 spawn / send_message / broadcast 管理团队</div>
  </div>
</div>

<div v-click class="p-4 rounded-xl bg-green-500/10 border-2 border-green-400/60">
  <div class="text-green-300 font-bold text-base mb-2">s16 团队协议</div>
  <div class="text-sm text-gray-300 space-y-1">
    <div>自由消息可以被无视，需要<span class="text-green-300">结构化握手</span></div>
    <div>每个请求带 request_id，响应必须关联同一个 id</div>
    <div>内置 shutdown 和 plan_approval 两套协议</div>
  </div>
</div>

<div v-click class="p-4 rounded-xl bg-orange-500/10 border-2 border-orange-400/60">
  <div class="text-orange-300 font-bold text-base mb-2">s17 自治智能体</div>
  <div class="text-sm text-gray-300 space-y-1">
    <div>全靠 Lead 分配任务，Lead 就成了<span class="text-orange-300">瓶颈</span></div>
    <div>WORK/IDLE 双阶段循环，空闲时自动扫描任务板认领</div>
    <div>认领需原子锁 + 角色匹配，压缩后自动补身份</div>
  </div>
</div>

<div v-click class="p-4 rounded-xl bg-purple-500/10 border-2 border-purple-400/60">
  <div class="text-purple-300 font-bold text-base mb-2">s18 Worktree 任务隔离</div>
  <div class="text-sm text-gray-300 space-y-1">
    <div>多人同目录工作，未提交改动<span class="text-purple-300">互相污染</span></div>
    <div>git worktree 给每个任务一个独立目录和分支</div>
    <div>Task 管"做什么"，Worktree 管"在哪做"，分开管理</div>
  </div>
</div>

</div>

<div v-click class="mt-6 text-center text-orange-500 text-lg">

核心循环始终不变，团队能力全部通过新增工具和外围机制实现
